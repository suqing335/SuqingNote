...
static int prveRoute = 0;
static const char* tmpLock = "incall_lock";
...
status_t AudioHardware::setMode(int mode)
{
    ...
    if (status == NO_ERROR) {
        ...
        if ((mMode != AudioSystem::MODE_IN_COMMUNICATION) && mVoipAudioMode) {
			//ALOGD("setMode() : (mMode != AudioSystem::MODE_IN_COMMUNICATION)&& mVoipAudioMode");
        ...
        if (mMode == AudioSystem::MODE_IN_CALL && !mInCallAudioMode) {
	    	...
	    	//ALOGD("setMode() : mMode == AudioSystem::MODE_IN_CALL && !mInCallAudioMode");
            if (mOutput != 0) {
                mLock.unlock();
		...
		prveRoute = 1;//add by cfj
            ALOGD("setMode() openPcmOut_l() prveRoute = %d", prveRoute);
            openPcmOut_l();
	...
	#ifdef JZX_INCALL_STATUS
		/*add by cfj 解决打电话时进入休眠后咪头没有声音进入的问题*/
		if ((jfd = open("/sys/power/wake_lock", O_WRONLY)) > 0) {
			jret = write(jfd, tmpLock, strlen(tmpLock));
			if (jret != strlen(tmpLock)) 
				ALOGE("setMode() /sys/power/wake_lock = %s, ret = %d, jfd = %d", tmpLock, jret, jfd);
			close(jfd);
		} else {
			ALOGE("open /sys/power/wake_lock  fiale !!!!\n");
		}	
	  /*  if ((jfd = open("/sys/class/leds/genesis-led/incallstatus", O_RDWR)) > 0) {
		jret = write(jfd, &tmp[0], 1);
		ALOGD("setMode() /sys/class/leds/genesis-led/incallstatus = %c, ret = %d, jfd = %d", tmp[0], jret, jfd);
		close(jfd);
	    } else {
		ALOGD("open /sys/class/leds/genesis-led/incallstatus  fiale !!!!\n");
	    }*/
	#endif    
        }
        ...
        if (mMode != AudioSystem::MODE_IN_CALL && mInCallAudioMode) {
            ...
            ALOGD("setMode() closePcmOut_l()");
            closePcmOut_l();
            mInCallAudioMode = false;
	#ifdef JZX_INCALL_STATUS    
	    //add by cfj
	    if ((jfd = open("/sys/power/wake_unlock", O_WRONLY)) > 0) {
		jret = write(jfd, tmpLock, strlen(tmpLock));
		if (jret != strlen(tmpLock))
			ALOGE("setMode() /sys/power/wake_unlock = %s, ret = %d, jfd = %d", tmpLock, jret, jfd);
		close(jfd);
	    } else {
		  ALOGE("open /sys/power/wake_unlock  fiale !!!!\n");  
	    }
	/*    if ((jfd = open("/sys/class/leds/genesis-led/incallstatus", O_RDWR)) > 0) {
		jret = write(jfd, &tmp[1], 1);		
		ALOGD("setMode() /sys/class/leds/genesis-led/incallstatus = %c, ret = %d, jfd = %d", tmp[1], jret, jfd);
		close(jfd);
	    } else {
		ALOGD("open /sys/class/leds/genesis-led/incallstatus  fiale !!!!\n");
	    }*/
		...
		/*add by cfj 解决播放音乐被电话打断后不能恢复声音的问题*/
		//ALOGD("setMode() : mMode == AudioSystem::MODE_NORMAL mPcmOpenCnt = %d", mPcmOpenCnt);
		if (mPcmOpenCnt == 1 && prveRoute != 0 && !mInCallAudioMode) {
			closePcmOut_l();
			openPcmOut_l();
			prveRoute = 0;
			ALOGD("setMode() : closePcmOut_l(), openPcmOut_l()");
		}
...
status_t AudioHardware::setParameters(const String8& keyValuePairs)
{
	...
	ALOGD("AudioHardware::setParameters() %s", keyValuePairs.string());
...
{
    ALOGD("setIncallPath_l: device %x", device);
...
unsigned AudioHardware::getOutputRouteFromDevice(uint32_t device)
{
    ...
    case AudioSystem::DEVICE_OUT_WIRED_HEADSHANK:
        if (mMode == AudioSystem::MODE_RINGTONE) return HEADSHANK_RINGTONE_ROUTE;
        else return HEADSHANK_NORMAL_ROUTE;
    ...
    case (AudioSystem::DEVICE_OUT_SPEAKER|AudioSystem::DEVICE_OUT_WIRED_HEADSHANK):
...
unsigned AudioHardware::getVoiceRouteFromDevice(uint32_t device)
{
    ...
    } else if (device & AudioSystem::DEVICE_OUT_WIRED_HEADSHANK) {
        if (mMode == AudioSystem::MODE_IN_CALL) return HEADSHANK_INCALL_ROUTE;
        else return HEADSHANK_VOIP_ROUTE;	
...
unsigned AudioHardware::getInputRouteFromDevice(uint32_t device)
{
    ...
    case AudioSystem::DEVICE_IN_WIRED_HEADSHANK:
        return HEADSHANK_MIC_CAPTURE_ROUTE;
    case AudioSystem::DEVICE_IN_WIRED_HEADPHONE:
        return HEADPHONE_MIC_CAPTURE_ROUTE;
...
{
	ALOGD("AudioHardware::getRouteFromDevice device %x", device);
    ...
    uint32_t *pChannels, uint32_t *pRate, AudioSystem::audio_in_acoustics acoustics)
{
    ...
    ALOGD("mInSampleRate %d, mSampleRate %d", mInSampleRate, mSampleRate);
...
{
    if (mPcm) {
        ALOGD("close_l() reset Capture MIC Path to OFF");
