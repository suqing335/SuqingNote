...
{
	ALOGE("is_playback_route() route %d", route);
    switch (route) {
    case MAIN_MIC_CAPTURE_ROUTE:
    case HEADPHONE_MIC_CAPTURE_ROUTE:
    case HEADSHANK_MIC_CAPTURE_ROUTE:
    ...
    case HEADSHANK_NORMAL_ROUTE:
    case HEADSHANK_INCALL_ROUTE:
    case HEADSHANK_RINGTONE_ROUTE:
    case HEADSHANK_VOIP_ROUTE:
...
{
    ALOGD("get_route_config() route %d", route);
	int fd = 0, ret;
	char tmp;
    if (!route_table) {
...
/*
    if (route == HANDS_FREE_MIC_CAPTURE_ROUTE || route == HEADSET_INCALL_ROUTE) {
	fd = open("/sys/class/leds/genesis-led/shank", O_RDONLY);
	if (fd > 0) {
		ALOGE("get_route_config() -----shank route = %s\n", (route == HANDS_FREE_MIC_CAPTURE_ROUTE) ? "HANDS_FREE_MIC_CAPTURE_ROUTE" : "HEADSET_INCALL_ROUTE");
		if (read(fd, &tmp, 1) == 1) {
			ALOGE("get_route_config() -----shank  = %c\n", tmp);
			if (tmp == '1') {
				route = (route == HANDS_FREE_MIC_CAPTURE_ROUTE) ? SHANK_MIC_CAPTURE_ROUTE : SHANK_INCALL_ROUTE; 
				ALOGE("get_route_config() -----route  = %s\n", (route == SHANK_MIC_CAPTURE_ROUTE) ? "SHANK_MIC_CAPTURE_ROUTE" : "SHANK_INCALL_ROUTE");
			}			
		}
		close(fd);
	}
    } else if (route == MAIN_MIC_CAPTURE_ROUTE) {
	fd = open("/sys/class/leds/genesis-led/bot", O_RDONLY);
	if (fd > 0) {
		ALOGE("get_route_config() -----bot route = MAIN_MIC_CAPTURE_ROUTE\n");
		if (read(fd, &tmp, 1) == 1) {
			ALOGE("get_route_config() -----bot  = %c\n", tmp);
			if (tmp == '1') {
				route = HEADPHONE_MIC_CAPTURE_ROUTE;
				ALOGE("get_route_config() -----route  = HEADPHONE_MIC_CAPTURE_ROUTE\n");
			}			
		}
		close(fd);
	}
    ...
    */
    ...
    switch (route) {
    case SPEAKER_NORMAL_ROUTE:
	ALOGE("get_route_config() SPEAKER_NORMAL_ROUTE ------------------------- \n");
        return &(route_table->speaker_normal);
    case SPEAKER_INCALL_ROUTE:
	ALOGE("get_route_config() SPEAKER_INCALL_ROUTE ------------------------- \n");
        return &(route_table->speaker_incall);
    case SPEAKER_RINGTONE_ROUTE:
	ALOGE("get_route_config() SPEAKER_RINGTONE_ROUTE ------------------------- \n");
        return &(route_table->speaker_ringtone);
    case SPEAKER_VOIP_ROUTE:
    	ALOGE("get_route_config() SPEAKER_VOIP_ROUTE ------------------------- \n");
        return &(route_table->speaker_voip);
    case EARPIECE_NORMAL_ROUTE:
	ALOGE("get_route_config() EARPIECE_NORMAL_ROUTE ------------------------- \n");
        return &(route_table->earpiece_normal);
    case EARPIECE_INCALL_ROUTE:
    	ALOGE("get_route_config() EARPIECE_INCALL_ROUTE ------------------------- \n");
        return &(route_table->earpiece_incall);
    case EARPIECE_RINGTONE_ROUTE:
    	ALOGE("get_route_config() EARPIECE_RINGTONE_ROUTE ------------------------- \n");
        return &(route_table->earpiece_ringtone);
    case EARPIECE_VOIP_ROUTE:
    	ALOGE("get_route_config() EARPIECE_VOIP_ROUTE ------------------------- \n");
        return &(route_table->earpiece_voip);
    case HEADPHONE_NORMAL_ROUTE:
	ALOGE("get_route_config() HEADPHONE_NORMAL_ROUTE ------------------------- \n");
        return &(route_table->headphone_normal);
    case HEADPHONE_INCALL_ROUTE:
    	ALOGE("get_route_config() HEADPHONE_INCALL_ROUTE ------------------------- \n");
        return &(route_table->headphone_incall);
    case HEADPHONE_RINGTONE_ROUTE:
    	ALOGE("get_route_config() HEADPHONE_RINGTONE_ROUTE ------------------------- \n");
        return &(route_table->headphone_ringtone);
    case SPEAKER_HEADPHONE_NORMAL_ROUTE:
    	ALOGE("get_route_config() SPEAKER_HEADPHONE_NORMAL_ROUTE ------------------------- \n");
        return &(route_table->speaker_headphone_normal);
    case SPEAKER_HEADPHONE_RINGTONE_ROUTE:
    	ALOGE("get_route_config() SPEAKER_HEADPHONE_RINGTONE_ROUTE ------------------------- \n");
        return &(route_table->speaker_headphone_ringtone);
    case HEADPHONE_VOIP_ROUTE:
    	ALOGE("get_route_config() HEADPHONE_VOIP_ROUTE ------------------------- \n");
        return &(route_table->headphone_voip);
    case HEADSET_NORMAL_ROUTE:
    	ALOGE("get_route_config() HEADSET_NORMAL_ROUTE ------------------------- \n");
        return &(route_table->headset_normal);
    case HEADSET_INCALL_ROUTE:
    	ALOGE("get_route_config() HEADSET_INCALL_ROUTE ------------------------- \n");
        return &(route_table->headset_incall);
    case HEADSET_RINGTONE_ROUTE:
    	ALOGE("get_route_config() HEADSET_RINGTONE_ROUTE ------------------------- \n");
        return &(route_table->headset_ringtone);
    case HEADSET_VOIP_ROUTE:
    	ALOGE("get_route_config() HEADSET_VOIP_ROUTE ------------------------- \n");
        return &(route_table->headset_voip);
    case HEADSHANK_NORMAL_ROUTE://
    	ALOGE("get_route_config() HEADSHANK_NORMAL_ROUTE ------------------------- \n");
        return &(route_table->headshank_normal);
    case HEADSHANK_INCALL_ROUTE:
    	ALOGE("get_route_config() HEADSHANK_INCALL_ROUTE ------------------------- \n");
        return &(route_table->headshank_incall);
    case HEADSHANK_RINGTONE_ROUTE:
    	ALOGE("get_route_config() HEADSHANK_RINGTONE_ROUTE ------------------------- \n");
        return &(route_table->headshank_ringtone);
    case HEADSHANK_VOIP_ROUTE:
    	ALOGE("get_route_config() HEADSHANK_VOIP_ROUTE ------------------------- \n");
        return &(route_table->headshank_voip);//
    case BLUETOOTH_NORMAL_ROUTE:
    	ALOGE("get_route_config() BLUETOOTH_NORMAL_ROUTE ------------------------- \n");
        return &(route_table->bluetooth_normal);
    case BLUETOOTH_INCALL_ROUTE:
    	ALOGE("get_route_config() BLUETOOTH_INCALL_ROUTE ------------------------- \n");
        return &(route_table->bluetooth_incall);
    case BLUETOOTH_VOIP_ROUTE:
    	ALOGE("get_route_config() BLUETOOTH_VOIP_ROUTE ------------------------- \n");
        return &(route_table->bluetooth_voip);
    case MAIN_MIC_CAPTURE_ROUTE:
    	ALOGE("get_route_config() MAIN_MIC_CAPTURE_ROUTE ------------------------- \n");
        return &(route_table->main_mic_capture);
    case HANDS_FREE_MIC_CAPTURE_ROUTE:
    	ALOGE("get_route_config() HANDS_FREE_MIC_CAPTURE_ROUTE ------------------------- \n");
        return &(route_table->hands_free_mic_capture);
    case BLUETOOTH_SOC_MIC_CAPTURE_ROUTE:
    	ALOGE("get_route_config() BLUETOOTH_SOC_MIC_CAPTURE_ROUTE ------------------------- \n");
        return &(route_table->bluetooth_sco_mic_capture);
    case PLAYBACK_OFF_ROUTE:
    	ALOGE("get_route_config() PLAYBACK_OFF_ROUTE ------------------------- \n");
        return &(route_table->playback_off);
    case CAPTURE_OFF_ROUTE:
    	ALOGE("get_route_config() CAPTURE_OFF_ROUTE ------------------------- \n");
        return &(route_table->capture_off);
    case INCALL_OFF_ROUTE:
    	ALOGE("get_route_config() INCALL_OFF_ROUTE ------------------------- \n");
        return &(route_table->incall_off);
    case VOIP_OFF_ROUTE:
    	ALOGE("get_route_config() VOIP_OFF_ROUTE ------------------------- \n");
        return &(route_table->voip_off);
    case HDMI_NORMAL_ROUTE:
    	ALOGE("get_route_config() HDMI_NORMAL_ROUTE ------------------------- \n");
        return &(route_table->hdmi_normal);
    case USB_NORMAL_ROUTE:
    	ALOGE("get_route_config() USB_NORMAL_ROUTE ------------------------- \n");
        return &(route_table->usb_normal);
    case USB_CAPTURE_ROUTE:
    	ALOGE("get_route_config() USB_CAPTURE_ROUTE ------------------------- \n");
        return &(route_table->usb_capture);
    case HEADSHANK_MIC_CAPTURE_ROUTE:
    	ALOGE("get_route_config() HEADSHANK_MIC_CAPTURE_ROUTE ------------------------- \n");
        return &(route_table->headshank_mic_capture);
    case HEADPHONE_MIC_CAPTURE_ROUTE:
    	ALOGE("get_route_config() HEADPHONE_MIC_CAPTURE_ROUTE ------------------------- \n");
        return &(route_table->headphone_mic_capture);
    default:
...
int route_set_controls(unsigned route)
{
	...
	route != HEADPHONE_MIC_CAPTURE_ROUTE &&
	route != HEADSHANK_MIC_CAPTURE_ROUTE &&
...
int route_pcm_close(unsigned route)
{
    ...
    ALOGD("route_pcm_close() route %d", route);
